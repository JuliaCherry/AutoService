...
const formBtnPrev = document.querySelector('.form__button-prev');
const formBtnNext = document.querySelector('.form__button-next');
const formBtnSubmit = document.querySelector('.form__button-submit');
const formTime = document.querySelector('.form__time');
const formFieldstType = document.querySelector('.form__fieldset-type');
const formFieldstDate = document.querySelector('.form__fieldst-date');
const formFieldstClient = document.querySelector('.form__fieldst-client');
const formFieldsets = [formFieldstType, formFieldstDate, formFieldstClient];
...
const formMonthWrapper = document.querySelector('.form__months');
  ...
  /* <p class="form__info form__info-type">Диагностика и&nbsp;ремонт двигателей</p>
              <p class="form__info">
                <time class="form__info-data" datetime="2024-03-02T14:00"
                  ...
                  <span class="form__info-data-time">14:00</span></time
                >
              </p> */
}
...
const showResultData = () => {
  ...
  <span class="form__info-data-day">${formatedDate}</span>
...
const updateFieldsetVisibility = () => {
  for (let i = 0; i < formFieldsets.length; i++) {
    if (i === currentStep) {
      formFieldsets[i].classList.add('form__fieldst-active');
    } else {
      formFieldsets[i].classList.remove('form__fieldst-active');
    }
  }
...
const createFormMonth = months => {
  ...
  formMonthWrapper.append(...buttonsMonth);
...
const handleInputForm = ({ currentTarget, target }) => {
  ...
  if (currentStep === 2) {
    const inputs = formFieldstClient.querySelectorAll('.form__input');
    let allFilled = true;
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      const response = await fetch(
        'https://basalt-forest-band.glitch.me/api/orders',
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        form.innerHTML = `
        <h2 class="contact__title";>Спасибо за заявку! Менеджер скоро свяжется с вами!</h2>`;
      } else {
